---
title: "Pricing"
description: "Per-request pricing with no token counting. Every response includes its cost."
---
...
**$0.03/req (ultra):** `gpt-5.4-pro` `claude-opus-4.6`

**$0.015/req (premium):** `gpt-5.4` `claude-sonnet-4.6`
...
```python
r = requests.get("https://www.ninjachat.ai/api/api-keys",
...
```python
r = requests.get("https://www.ninjachat.ai/api/api-keys/transactions",
    headers={"Authorization": f"Bearer {os.environ['NINJACHAT_API_KEY']}"})
...
```python
# Recent logs
r = requests.get("https://www.ninjachat.ai/api/api-keys/logs",
    headers={"Authorization": f"Bearer {os.environ['NINJACHAT_API_KEY']}"},
...
```bash
curl "https://www.ninjachat.ai/api/api-keys/logs?format=csv&from=2026-03-01" \
  -H "Authorization: Bearer nj_sk_YOUR_API_KEY" \
  -o usage.csv
```

For rate limits per endpoint, see [Rate Limits](/rate-limits).
